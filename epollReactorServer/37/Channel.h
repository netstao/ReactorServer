...
private:
    ...
    EventLoop *eloop_;   //Channel 对应的红黑色树，channel与epoll 多对一的关系，一个channel只对应一个Epoll, 一个Epoll对用多个Channel 有服务端监听channel 新的客户端连接的channel 高发可能有N多
    ...
    std::function<void()> closecallback_; //客户端fd关闭回调 回调Connecttion::closecallback()
    std::function<void()> errorcallback_; //客户端fd发生错误回调函数 回调Connecttion::errorcallback()
    std::function<void()> writecallback_; //客户端fd发生写事件回调函数 回调Connecttion::writecallback()

public:
    Channel(EventLoop *eloop, int fd);   //Channel是Acceptor和Connection的下层类。
    ~Channel();

    int fd();       // 返回fd_成员
    void useet();    //边缘触发模式
    void enablereading();  //让epoll_wait()监视fd_的读事件
    void disablereading();  //让epoll_wait()取消fd_的读事件
    void enablewriteing();  //让epoll_wait()注册fd_的写事件
    void disablewriteing();  //让epoll_wait()取消fd_的写事件
    ...
    void setclosecallback(std::function<void()>  fn); //客户端fd关闭回调 回调Connecttion::closecallback()
    void seterrorcallback(std::function<void()>  fn); //客户端fd发生错误回调函数 回调Connecttion::errorcallback()
    void setwritecallback(std::function<void()>  fn); //客户端fd发生写事件回调函数 回调Connecttion::errorcallback()
};
