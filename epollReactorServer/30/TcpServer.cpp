...
TcpServer::TcpServer(const std::string &ip, const uint16_t port, int threadnum)
          :threadnum_(threadnum),maineloop_(new EventLoop),acceptor_(maineloop_, ip, port)
...
//新连接
 void TcpServer::newconnection(std::unique_ptr<Socket> clientsock) 
 {
    //  Connection *conn = new Connection(maineloop_, clientsock); //new出来的对象没有释放 以后解决
     spConnection conn(new Connection(subeloops_[clientsock->fd()%threadnum_], std::move(clientsock))); //new出来的对象没有释放 以后解决
